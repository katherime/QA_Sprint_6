...
class TestOrderPageScooter:
    ...
    @allure.description(
        'Переходим по кнопке оформления заказа в блоке header, проверяем, что открылась страница с оформлением заказа')
    ...
    @allure.description(
        'Переходим по оформления кнопке заказа в блоке main, проверяем, что открылась страница с оформлением заказа')
    ...
    @allure.title(
        'Проверка заполнения формы с валидными значениями и создание страницы с номером заказа при переходе через кнопку заказа в header')
    @allure.description(
        'Переходим по кнопке оформления заказа в header, заполняем валидные данные и подтверждаем оформление заказа, затем проверяем что произошел переход на страницу статуса заказа')
    ...
    def test_fill_the_form_correct_data_through_header_button(self, driver, name, lastname, address,
                                                              metro, telephone, time_order, rental_period, color,
                                                              commentary):
    ...
    @allure.title(
        'Проверка заполнения формы с валидными значениями и создание страницы с номером заказа при переходе через кнопку заказа в main')
    @allure.description(
        'Переходим по кнопке оформления заказа в main, заполняем валидные данные и подтверждаем оформление заказа, затем проверяем что произошел переход на страницу статуса заказа')
    ...
    @allure.title(
        'Проверяем, что при успешном заказе появляется всплывающее окно с сообщением об успешном создании заказа.')
    @allure.description(
        'Заполняем форму на оформление заказа и проверяем, что появилось всплывающее окно с текстом об успешном оформлении заказа')
    ...
    @allure.title(
        'Проверка полного флоу при заказе самоката через кнопку в header, и затем после оформления, проверяем переход на страницу Яндекс Дзена через логотип')
    @allure.description(
        'Заполняем форму на оформление заказа и проверяем, что после перехода на страницу оформленного заказа, при тапе по логотипу Яндекса происходит переход на страницу Яндекс Дзена')
    ...
    @allure.title(
        'Проверка полного флоу при заказе самоката через кнопку в main, и затем после оформления, проверяем переход на главную страницу Самоката через логотип')
    @allure.description(
        'Заполняем форму на оформление заказа и проверяем, что после перехода на страницу оформленного заказа, при тапе по логотипу Самоката происходит переход на страницу сайта заказа Самокатов')
    ...
    def test_successful_order_through_header_button_to_main_scooter_page(self, driver, name, lastname, address,
                                                                         metro, telephone, time_order, rental_period,
                                                                         color, commentary):
        order_page = OrderPage(driver)
        self.test_transition_through_main_div_button_to_order_page(driver)
        order_page.fill_the_form_for_order_scooter(name, lastname, address, metro, telephone, time_order, rental_period,
